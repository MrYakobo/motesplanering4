...
(function (exports) {
  ...
  function buildPersonalEmail(ev, contact, db, schedules) {
    ...
    var orgName = (db.settings && db.settings.orgName) || '';
    var orgLogo = (db.settings && db.settings.orgLogo) || '';
    var baseUrl = (db.settings && db.settings.baseUrl) || '';
    ...
    var html = '<div style="font-family:\'Be Vietnam\',sans-serif;background:#fff;padding:20px;border-radius:3px">'
      ...
      + (contact.token ? '<p style="font-size:12px;color:#666">Uppdatera dina uppgifter eller teammedlemskap: <a href="' + (baseUrl || '') + '/?token=' + contact.token + '">Min profil</a></p>' : '')
      + (orgLogo ? '<img style="width:150px" src="' + orgLogo + '">' : '')
      + '</div>'
      + '<div style="text-align:center;margin-top:10px;font-size:12px;color:#999">' + orgName + '<br>Det här mailet avser ' + ev.date + ' ' + (ev.time || '') + '.<br>Ser något fel ut? Svara på det här mailet isåfall.</div>';
  ...
  function buildMailchatEmail(ev, db, schedules) {
    ...
    var html = '<div style="font-family:\'Be Vietnam\',sans-serif;background:#fff;padding:20px;border-radius:3px">'
      ...
      + '<h3 style="font-weight:400">Fortsatt fin vecka!</h3>'
      + (orgLogo ? '<img style="width:150px" src="' + orgLogo + '">' : '')
      + '</div>'
      + '<div style="text-align:center;margin-top:10px;font-size:12px;color:#999">' + orgName + '<br>Det här mailet avser ' + ev.date + ' ' + (ev.time || '') + '.<br>Ser något fel ut? Svara på det här mailet isåfall.</div>';
